truncate table  yp_dws.dws_user_daycount;
----presto--------------------
-- 增加每个节点的最大writer数
SET SESSION task_writer_count = 2;  -- 默认通常是1
-- 增加总的open writers限制（主要参数）
SET SESSION max_open_writers_for_partition = 200;  -- 默认是100
...
(
    ...
    select
           ...
           0 order_count, 0 order_amount,
           0 payment_count, 0 payment_amount,
           user_id, substring(dt,1,7) as dt
